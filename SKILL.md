...
4. **长文处理**：中等内容、长文或复杂材料必须读取 [`references/long-form-workflow.md`](references/long-form-workflow.md)；长文还要读取 [`references/large-canvas-workflow.md`](references/large-canvas-workflow.md)，先做信息保全清单和大画布分区草稿；不要直接把全文塞进 SVG。
...
7. **稳定渲染**：如果版式是结论先行、问题拆解或长文大画布，读取 [`references/deterministic-rendering.md`](references/deterministic-rendering.md)，先生成 JSON brief，再用 `scripts/render-whiteboard.mjs` 生成 SVG；不要自由手写整张 SVG。
...
- 信息太多时，先做信息保全清单，再用分区承载；不要把原文完整搬上画板，也不要丢掉关键结论、约束、风险、指标、证据和行动。
- 长文默认生成一个纵向大画布；总览是第一屏，不是完整输出。
- 如果单屏超过容量预算，不要靠缩小字号硬塞；扩展为下一分区或增高画布。
