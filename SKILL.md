...
7. **稳定渲染**：如果版式是结论先行、问题拆解或长文 onepage 大画布，读取 [`references/deterministic-rendering.md`](references/deterministic-rendering.md)，先生成 JSON brief，再用 `scripts/render-whiteboard.mjs` 生成 SVG；不要自由手写整张 SVG。
