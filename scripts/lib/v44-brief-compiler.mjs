...
function expressionBlock(region, facts) {
  ...
  if (type === "metric-card") {
    ...
    return { type, title: clip(fact?.text || "核心指标", 18), value: clip(fact?.value || fact?.text.match(/[-+]?\d+(?:\.\d+)?%?/)?.[0] || "待观察", 12), label: "阶段结果", note: clip(fact?.text, 24), status: "neutral", sourceFactIds: region.factIds };
  }
...
export function compileV44Brief({ semanticModel, planningResult, style = "linear-system", title }) {
  ...
  const blocks = ensureExpressionRequirements(selected.regions.map((region) => expressionBlock(region, facts)), semanticModel);
...
function compileFlowBrief(model, candidate, style, title) {
  ...
  const nodes = ordered.map((fact, index) => ({ id: `node-${index + 1}`, title: clip(fact.text, 14), body: [clip(fact.text, 24)], type: index === 0 ? "start" : index === ordered.length - 1 ? "result" : fact.type === "constraint" ? "decision" : "action", step: index + 1, status: fact.type === "constraint" ? "risk" : "neutral", sourceFactIds: [fact.id] }));
  const edges = nodes.slice(0, -1).map((node, index) => ({ from: node.id, to: nodes[index + 1].id, label: "进入下一步" }));
  return { pipelineVersion: "4.4", engine: "v4", renderTarget: "svg", layout: "flow-canvas", style, flowMode: "linear-flow", title: clip(title || model.facts[0].text, 32), subtitle: "按输入、动作、判断和输出组织流程", summaryLabel: "流程判断", summary: clip(model.facts[0].text, 90), flowNodes: nodes, flowEdges: edges, planning: { inventoryId: model.inventoryId, selectedFactIds: ordered.map((fact) => fact.id), omittedFacts: model.facts.filter((fact) => !ordered.includes(fact)).map((fact) => ({ id: fact.id, reason: "deferred-to-detail" })), routeDecisionId: candidate.planId } };
}
