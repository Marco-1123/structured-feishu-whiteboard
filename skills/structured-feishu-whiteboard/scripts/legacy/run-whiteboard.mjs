...
if (!inventoryPath || !routePath || !briefPath || !outputDir) {
  console.error("legacy regression only: node scripts/legacy/run-whiteboard.mjs --inventory inventory.json --route route.json --brief brief.json --output-dir output [--skip-whiteboard-cli]");
  process.exit(1);
}
